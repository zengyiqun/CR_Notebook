...
- [x] 跨天事件连续条形渲染（按周分组 + lane 分配，避免中断显示）
- [x] 双击空白新增事件 / 双击事件编辑
- [x] 颜色标记
...
- [x] 个人 / 组织用量仪表盘
- [x] 可选时间范围（7 / 30 / 60 / 90 天）
- [x] 可视化图表
- [x] 活动趋势图在长周期（30/60/90 天）下的柱形宽度与标签密度自适应修复
...
- [x] README.md 完整文档
- [x] PLAN.md 开发计划文档
- [x] README 增加系统截图与数据库设计说明（表结构概览 + ER 图）
- [x] 前后端关键复杂逻辑补充必要注释（导出转换、跨天排布、拖拽重排、多租户上下文）
...
| V7 | `V7__folder_sort_order.sql` | 文件夹新增 `sort_order` 字段，支持手动拖拽排序 |
...
- 开发环境统一采用**本地 MySQL**（`127.0.0.1:3306`）直连，降低容器依赖和环境漂移问题
- `application.yml` 默认提供可运行参数（root / `123@abc`），按本机实际配置覆盖
- Flyway 在应用启动时自动迁移，确保本地数据库结构与代码版本一致
